...
__global__ void Accuracy_MultinomialLogistic(size_t CUDA_NUM_LOOPS, size_t N, int C, int M, size_t wN, const StorageT* pred, const StorageT* label, const StorageT* weight, const StorageT* weightTensor, StorageT* loss){
	...
	for (size_t idx = idxBase; idx < min(N,idxBase+CUDA_NUM_LOOPS); ++idx ){
		...
		//printf("N is %d, C is %d, M is %d, wN is %d, l is %d, baseID is %d, elementID is %d\n",N,C,M,wN,l,baseID,elementID);
		//printf("label value %d is %f\n",idx, label[idx]);
		//printf("softmax value %d is %f, softmax value 0 is %f\n",elementID, pred[elementID], pred[0]);
		//printf("prob value %d is %f\n",elementID, prob);
		for(int d=0;d<C;++d){
...
public:
  ...
  public:
    ...
    void shuffle(){
      ...
      if(darts_file != "") {
	dart_names = permute_names(v, dart_names);
      }
	...
	checkCUDA(__LINE__, cudaMemcpy(out[1]->dataGPU, labelCPU->CPUmem, batch_size * labelCPU->sizeofitem() * sizeofStorageT, cudaMemcpyHostToDevice) );
      ...
      if(verbose && num_pockets_per_pdb != 0) {
	for(int i = 0; i<batch_size * labelCPU->sizeofitem(); i++) {
	  for(int l = 0; l<num_pockets_per_pdb; l++) {
	    std::cout<<"label "<<i+l<<" is "<<labelCPU->CPUmem[i*num_pockets_per_pdb+l]<<std::endl;
	    std::cout<<"pocket coordinates are "<<bbCPU->CPUmem[i*num_pockets_per_pdb*7+l*7+0]<<" "<<bbCPU->CPUmem[i*num_pockets_per_pdb*7+l*7+1]<<" "<<bbCPU->CPUmem[i*num_pockets_per_pdb*7+l*7+2]<<" "<<bbCPU->CPUmem[i*num_pockets_per_pdb*7+l*7+3]<<" "<<bbCPU->CPUmem[i*num_pockets_per_pdb*7+l*7+4]<<" "<<bbCPU->CPUmem[i*num_pockets_per_pdb*7+l*7+5]<<" "<<bbCPU->CPUmem[i*num_pockets_per_pdb*7+l*7+6]<<std::endl;
	  }
	}
      }
	...
	R3Affine rotation = pdbrotate::GetPDBRotation(curr_pdb);
	PDBFile* rot_pdb = new PDBFile(*curr_pdb);
	rot_pdb->Transform(rotation);
	...
	if(num_pockets_per_pdb != 0) {
	  ...
	  if(darts_file != "") {
	    ...
	    /*
	    //Create ligsite grid to get worldtogrid transformation for pre-generated points.
	    ...
	    */
	    ...
	    for(int i = 0; i < dartlist->darts.NEntries(); i++) {
	      ...
	      //Rotate dart to match rotated pdb.
	      dartlist->darts.Kth(i)->position.Transform(rotation);
	  ...
	  if(verbose) {
	    ...
	    std::cout<<"total negatives is "<<total_negatives<<std::endl;
	    std::cout<<"num positives to select is "<<num_positives_to_select<<std::endl;
	    std::cout<<"num negatives to select is "<<num_negatives_to_select<<std::endl;
	  }


	  //If we do not have enough negatives or positives, duplicate them.
	  if(total_positives != 0 && total_positives < num_positives_to_select) {
	    int diff = num_positives_to_select - total_positives;
	    int currind = 0;
	    int num_added = 0;
	  ...
	  else if(total_negatives  != 0 && total_negatives < num_negatives_to_select) {
	   int diff = num_negatives_to_select - total_negatives;
	    int currind = 0;
	    int num_added = 0;
	    while(num_added < diff) {
