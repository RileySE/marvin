...
*/
	  ...
	  //generate darts using ligsite
	    ...
	    if(num_negatives_per_positive == -1) {
	      ...
	      darts = grd2drt::CreateDarts(ligsitegrid, num_pockets_per_pdb*10, grd2drt::min_spacing, grd2drt::min_cavity_radius, grd2drt::min_cavity_volume);
	    }
	  ...
	  for(int dind = 0; dind < darts->NEntries(); dind++) {
	    ...
	    */
	    ...
	    if(num_negatives_per_positive == -1 || (labelval == 1 && num_positives_seen < num_positives_to_select)) {
	    ...
	    else if(labelval == 0 && num_negatives_included < num_pockets_per_pdb - num_positives_to_select) { //If negative, include if we have less than the total quota.
